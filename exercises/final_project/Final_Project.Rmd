...
bank <- bank %>% 
  mutate(age_distribution = cut(age, c(20, 30,40, 50, 60, 70, 80, 90)))
summary(bank)



g3 <- ggplot(bank) + 
  geom_bar(aes(x = age_distribution)) +
  facet_grid(. ~ Outcome) +
  theme_few() +
  coord_flip()
...
bank <- select(bank, -default, -nr.employed, -loan,-duration, -pdays)
bank = arrange(bank, y)
N = nrow(bank)
...
load_train = bank[train_ind,]
load_test = bank[-train_ind,]
...
library(rpart)
library(rpart.plot)
fit.tree <- rpart(y~., data = bank, method = 'class')
